...
// student add
app.put('/studentAdd',async (req, res) => {
  ...
  const sqlQuery = 'INSERT INTO student_details (name,regno,department,pass_year,Domain,college_name,image,phone,email) values(?,?,?,?,?,?,"null","null","null")';
  const values = [name, id, department, year, domain, "MKCE"];

  db.query(sqlQuery, values, async (err, results) => {
    if (err) {
      console.log(err);
      return res.status(500).json({ error: 'Internal server error' });
    }
    const data = await prisma.student_login.create({
      data:{
        name:name,
        pass:name
      }
...
//staff details add
app.put('/staffAdd', async (req, res) => {
  const name = req.body.name;
  const id = req.body.id;
  const DOJ = req.body.DOJ;
  const email = req.body.email;
  ...
  const sqlQuery = 'INSERT INTO staff_details (staff_name,staff_id,staff_phone,staff_email,Domain,DOJ,college_name,department,image) values(?,?,?,?,?,?,?,?,"null")';
  ...
  const data = await prisma.staff_login.create({
    data:{
      name:name,
      pass:name
    }
...
app.get("/alumniProfile/:name", (req, res) => {
  const name = req.params.name;
  console.log(name)
  const sqlQuery = `SELECT * FROM alumni_details where name =  ? `;
  const values = [name];
  db.query(sqlQuery, values, (err, results) => {
    if (err) {
      console.log(err);
...
// staff data profile
app.get("/staffProfile/:name", (req, res) => {
  const name = req.params.name;
  const sqlQuery = `SELECT * FROM staff_details where staff_name=?   `;
  const value = [name]
